...
Where dayOfWeek is: 0=Sunday, 1=Monday, 2=Tuesday, 3=Wednesday, 4=Thursday, 5=Friday, 6=Saturday`
  ...
  // "Plan It" workflow - generates meal plan, all recipes, and shopping list in one go
  app.post("/api/kitchen/plan-it", isAuthenticated, async (req: Request, res: Response) => {
    try {
      const userId = (req.user as any)?.claims?.sub;
      if (!userId) return res.status(401).json({ error: "Unauthorized" });
      ...
      const sendProgress = (step: string, status: string, data?: any) => {
        res.write(`data: ${JSON.stringify({ step, status, ...data })}\n\n`);
      };

      // Step 1: Generate meal plan
      sendProgress("meal_plan", "started");
      ...
      const mealResponse = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: `You are a helpful meal planning assistant. Generate a diverse, family-friendly weekly dinner plan. ${cookbookContext}`
          },
          {
            role: "user",
            content: `Create a weekly dinner plan (Monday-Sunday). Available ingredients: ${ingredientList}. Return JSON: {"meals": [{"dayOfWeek": 1, "mealName": "...", "notes": "cook time"}]}`
          }
        ],
        response_format: { type: "json_object" },
        max_completion_tokens: 1024,
      ...
      let meals: { dayOfWeek: number; mealName: string; notes?: string }[] = [];
      try {
        const parsed = JSON.parse(mealResponse.choices[0]?.message?.content || "{}");
        meals = parsed.meals || parsed.mealPlan || parsed.plan || [];
        if (Array.isArray(parsed) && !meals.length) meals = parsed;
      } catch {
        meals = [];
      }
      meals = meals.filter(m => typeof m.dayOfWeek === 'number' && typeof m.mealName === 'string');
      ...
      if (meals.length === 0) {
        const mealOptions = [
          ["Grilled Chicken Salad", "Honey Garlic Chicken", "Lemon Herb Chicken"],
          ["Spaghetti Carbonara", "Pasta Primavera", "Mushroom Pasta"],
          ["Beef Tacos", "Beef Stir-Fry", "Shepherd's Pie"],
          ["Grilled Salmon", "Fish Tacos", "Shrimp Scampi"],
          ["Homemade Pizza", "Veggie Burgers", "Quesadillas"],
          ["BBQ Ribs", "Pulled Pork", "Grilled Steak"],
          ["Sunday Roast", "Lasagna", "Baked Ham"],
        ];
        meals = [1, 2, 3, 4, 5, 6, 0].map((day, idx) => ({
          dayOfWeek: day,
          mealName: mealOptions[idx][Math.floor(Math.random() * mealOptions[idx].length)],
          notes: ["25 min", "30 min", "35 min", "40 min", "45 min", "1 hour", "2 hours"][idx],
        }));
      }

      const plan = await storage.createMealPlanForWeek({ userId, weekStartDate: targetWeekStartDate });
      const mealDays: { id: number; dayOfWeek: number; mealName: string }[] = [];
      
      for (const meal of meals) {
        const day = await storage.addMealPlanDay({
          mealPlanId: plan.id,
          dayOfWeek: meal.dayOfWeek,
          mealName: meal.mealName,
          notes: meal.notes || null,
        });
        mealDays.push({ id: day.id, dayOfWeek: day.dayOfWeek, mealName: day.mealName });
      }

      sendProgress("meal_plan", "completed", { meals: mealDays.map(d => d.mealName) });

      // Step 2: Generate all recipes
      sendProgress("recipes", "started", { total: mealDays.length });

      const allIngredients: { name: string; quantity: string; category: string }[] = [];
      
      for (let i = 0; i < mealDays.length; i++) {
        const day = mealDays[i];
        sendProgress("recipe", "generating", { index: i, mealName: day.mealName });

        const recipePrompt = `Generate a complete recipe for: ${day.mealName}

Return JSON format:
{
  "recipe": "Full recipe markdown with # Title, ## Ingredients list (- quantity ingredient), ## Instructions",
  "ingredients": [{"name": "ingredient name lowercase singular", "quantity": "amount with unit", "category": "produce|meat|dairy|pantry|frozen|bakery|beverages|other"}],
  "imagePrompt": "Professional food photography prompt for this dish"
}`;
        ...
        try {
          const recipeResponse = await openai.chat.completions.create({
            model: "gpt-4.1",
            messages: [
              { role: "system", content: "You are a helpful chef. Generate complete recipes with ingredient lists." },
          ...
          const recipeContent = recipeData.recipe || `# ${day.mealName}\n\nRecipe content will be generated when you view this recipe.`;
          const imagePrompt = recipeData.imagePrompt || `Professional food photography of ${day.mealName}. Photorealistic, appetizing, warm lighting.`;

          await storage.updateMealPlanDay(day.id, { recipeContent, recipeImagePrompt: imagePrompt });
          ...
          sendProgress("recipe", "completed", { index: i, mealName: day.mealName });
        } catch (recipeError) {
          console.error(`Error generating recipe for ${day.mealName}:`, recipeError);
          sendProgress("recipe", "error", { index: i, mealName: day.mealName });
        }
      }

      sendProgress("recipes", "completed");

      // Step 3: Create shopping list from all ingredients
      sendProgress("shopping_list", "started");

      // Deduplicate and merge ingredients
      const mergedIngredients = new Map<string, { quantity: string; category: string }>();
      for (const ing of allIngredients) {
        const key = ing.name.toLowerCase().trim();
        if (!mergedIngredients.has(key)) {
          mergedIngredients.set(key, { quantity: ing.quantity, category: ing.category });
        } else {
          // Keep the first quantity, but could merge quantities in a smarter way later
          const existing = mergedIngredients.get(key)!;
          mergedIngredients.set(key, { ...existing });
        }
      }

      // Filter out ingredients user already has
      const existingIngredientNames = new Set(ingredients.map(i => i.name.toLowerCase()));
      const shoppingItems = Array.from(mergedIngredients.entries())
        .filter(([name]) => !existingIngredientNames.has(name))
        .map(([name, data]) => ({
      ...
      // Create shopping list for this week
      const shoppingList = await storage.createShoppingListForWeek({
        userId,
        name: `Week of ${targetWeekStartDate}`,
        weekStartDate: targetWeekStartDate,
      });
      ...
      sendProgress("shopping_list", "completed", { itemCount: shoppingItems.length });

      // Done!
      sendProgress("done", "completed", { 
        mealPlanId: plan.id, 
        shoppingListId: shoppingList.id,
        weekStartDate: targetWeekStartDate 
      });
      ...
      console.error("Error in Plan It workflow:", error);
      if (res.headersSent) {
        res.write(`data: ${JSON.stringify({ step: "error", status: "failed", error: "Workflow failed" })}\n\n`);
        res.end();
      } else {
        res.status(500).json({ error: "Failed to execute Plan It workflow" });
      }
    }
  });
  ...
  app.get("/api/kitchen/shopping-lists", isAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      const listsWithItems = await Promise.all(lists.map(async (list) => {
        const fullList = list.weekStartDate 
          ? await storage.getShoppingListByWeek(userId, list.weekStartDate)
          : null;
        return fullList || { ...list, items: [] };
      }));
